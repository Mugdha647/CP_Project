...
// Default initial roadmap data to seed MongoDB if empty
const defaultStages = [
  {
    id: "stage-1",
    level: "Beginner",
...
// ====================================
// GET ROADMAP (Public / User)
// ====================================
const getRoadmap = async (req, res) => {
  try {
    let stages = await RoadmapStage.find().sort({ order: 1 });

    // Auto-seed if database collection is empty
    if (stages.length === 0) {
      await RoadmapStage.insertMany(defaultStages);
      stages = await RoadmapStage.find().sort({ order: 1 });
    }
...
// ====================================
// SAVE / SYNC ENTIRE ROADMAP (Admin Only)
// ====================================
const saveFullRoadmap = async (req, res) => {
  try {
    ...
    // Replace all stages with updated array
    await RoadmapStage.deleteMany({});
    const inserted = await RoadmapStage.insertMany(stages);
    ...
    console.error("Error saving full roadmap:", error);
...
const addStage = async (req, res) => {
  try {
    const { title, description, level, levelColor, topics } = req.body;

    const count = await RoadmapStage.countDocuments();
    const id = `stage-${Date.now()}`;

    const newStage = await RoadmapStage.create({
      id,
      title,
      description: description || "",
      level: level || "Beginner",
      levelColor: levelColor || (level === "Advanced" ? "#8b5cf6" : level === "Intermediate" ? "#3b82f6" : "#10b981"),
      order: count + 1,
      topics: topics || []
    });
...
const updateStage = async (req, res) => {
  try {
    const { id } = req.params;
    const stage = await RoadmapStage.findOneAndUpdate({ id }, req.body, { new: true });
...
const deleteStage = async (req, res) => {
  try {
    const { id } = req.params;
    const stage = await RoadmapStage.findOneAndDelete({ id });
...
// ====================================
// ADD TOPIC TO STAGE (Admin Only)
// ====================================
const addTopic = async (req, res) => {
  try {
    const { stageId } = req.params;
    const { name, summary, resources, problems } = req.body;

    const stage = await RoadmapStage.findOne({ id: stageId });
    if (!stage) {
      return res.status(404).json({ message: "Stage not found" });
    }
    ...
    await stage.save();
    res.status(201).json({ message: "Topic added successfully", stage });
  } catch (error) {
...
// ====================================
// DELETE TOPIC FROM STAGE (Admin Only)
// ====================================
const deleteTopic = async (req, res) => {
  try {
    const { stageId, topicId } = req.params;
    const stage = await RoadmapStage.findOne({ id: stageId });
    ...
    stage.topics = stage.topics.filter((t) => t.id !== topicId);
    await stage.save();

    res.json({ message: "Topic deleted successfully", stage });
  } catch (error) {
